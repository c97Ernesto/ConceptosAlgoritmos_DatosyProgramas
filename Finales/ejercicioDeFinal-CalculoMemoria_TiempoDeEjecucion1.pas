...
VAR							//MEMORIA ESTÁTICA: Es la suma delas variables declaradas en VAR
	v: vec;			
	{"v" de tipo array de 500 pos * 4 por ser cada pos de tipo puntero): 500  * 4 = 2000 bytes}
	
	l,aux: lista;			
	{"l" y "aux" de tipo puntero: 4 + 4 = 8 bytes}
	emp: empleado;				
	{"emp" de tipo registro(la suma de sus campos string + integer + real): (35+1) + 6 + 8 = 50bytes}
	
	i: integer;		
	{"i" de tipo integer: 6 bytes}		
	
{TOTAL de memoria ESTÁTICA = 2000 + 8 + 50 + 6 = 2064 bytes}

BEGIN						
{MEMORIA DINÁMICA: Se empieza a calcular desde el primer new(), (siempre y cuando se encuentre en el programa)
y con el dispose() liberamos memoria.}
	l:=nil;
...
BEGIN
	...
	for i:=1 to 10 do begin   				{(3*10+2) = 32ut}
		...
		new(aux); 				{no cuenta para el tiempo de ejecución}
...
END.
{TOTAL de TIEMPO DE EJECUCIÖN = 1 + 32 + 30 + 20 + 10 + 10 + 10 = 113ut como maximo
